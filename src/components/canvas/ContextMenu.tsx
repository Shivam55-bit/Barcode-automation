    ...
    >
      {element ? (
        <>
          ...
          {onBringForward && <ContextItem icon={<ArrowUp className="w-3.5 h-3.5" />} label="Bring Forward" shortcut="Ctrl+]" onClick={() => { onBringForward(); onClose(); }} />}
          {onSendBackward && <ContextItem icon={<ArrowDown className="w-3.5 h-3.5" />} label="Send Backward" shortcut="Ctrl+[" onClick={() => { onSendBackward(); onClose(); }} />}
