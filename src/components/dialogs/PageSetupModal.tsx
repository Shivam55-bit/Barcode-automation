...
export const PageSetupModal: React.FC<PageSetupModalProps> = ({
  ...
  const [orientation, setOrientation] = useState<'portrait' | 'landscape' | 'portrait-180' | 'landscape-180'>('portrait');
  ...
  // Shape
  const [labelShape, setLabelShape] = useState<'rectangle' | 'rounded' | 'oval'>('rectangle');
  ...
  useEffect(() => {
    if (template && isOpen) {
      ...
      const existingShape = (template as any).shape || (template.dimensions as any)?.shape;
      if (existingShape === 'oval' || existingShape === 'circle' || existingShape === 'ellipse') {
        setLabelShape('oval');
      } else if (existingShape === 'rounded' || existingShape === 'round-rectangle') {
        setLabelShape('rounded');
      } else {
        setLabelShape('rectangle');
      }

      const existingRadius = (template as any).cornerRadius ?? (template.dimensions as any)?.cornerRadius;
      if (existingRadius !== undefined) {
        setCornerRadiusMm(existingRadius);
      }
      ...
      if (template.mediaType) {
        if (template.mediaType === 'continuous') setSensorType('continuous');
        else if (template.mediaType === 'black_mark') setSensorType('mark');
        else setSensorType('gap');
      }
    }
  }, [template, isOpen]);
  ...
  const handleApply = () => {
    const effectiveMediaType: 'gap' | 'continuous' | 'black_mark' =
      sensorType === 'continuous' ? 'continuous' : sensorType === 'mark' ? 'black_mark' : 'gap';
    ...
    onApplyPageSetup({
      ...
      margins: {
        ...
        right: marginRight,
        bottom: marginBottom,
        bleed: template.margins?.bleed || 1,
        safeZone: template.margins?.safeZone || 2,
        unit: 'mm',
      },
      sheetGrid: {
